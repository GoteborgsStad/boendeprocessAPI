...
if (isset($_GET['slo'])) {
    ...
    curl_setopt($ch, CURLOPT_POSTFIELDS, 'personal_identity_number=' . $personalIdentityNumber . '&password=secret&login_key=' . env('LOGIN_KEY'));
    ...
    if (json_decode(base64_decode($payload))->role === 'AU') {
        header('Location: ' . $feBaseUrl . '?appToken=' . json_decode($serverOutput)->token);
    } else {
