...
class AssignmentCategoriesTableSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        factory(\App\AssignmentCategory::class, count($assignmentCategoryNames))->create()->each(function ($f) use(&$assignmentCategoryNames, &$assignmentCategoryDescriptions, &$assignmentCategoryColors) {
            $f->name        = array_shift($assignmentCategoryNames);
            $f->description = array_shift($assignmentCategoryDescriptions);
            $f->color       = array_shift($assignmentCategoryColors);
